...
side_layer1_ctx.fillRect(
  0,
  0,
  side_layer1_ctx.canvas.width,
  side_layer1_ctx.canvas.height
);
...
function drawColorLayer(color = `#ff0000`) {
  ...
  layer1_gr.addColorStop(0.95, color);
...
function drawBlackNWhiteLayer() {
  ...
  layer1_gr2.addColorStop(0.95, "rgba(0,0,0,1)");
  layer1_gr2.addColorStop(0.5, "rgba(0,0,0,0.2)");
  layer1_gr2.addColorStop(0.4, "rgba(0,0,0,0.1)");
  layer1_gr2.addColorStop(0.1, "rgba(0,0,0,0)");
...
function setCurrentColor() {
  currentColor = layer1_ctx.getImageData(mainSwitchPos.x, mainSwitchPos.y, 1, 1)
    .data;
  alpha = (currentColor[3] / 255).toFixed(2);
  currentColor = rgbaToRgb(
    currentColor[0],
    currentColor[1],
    currentColor[2],
    alpha
  ...
  if (!sideColor) {
    sideColor = side_layer1_ctx.getImageData(
      sideSwitchPos.x,
      sideSwitchPos.y,
      1,
      1
...
function setLogoColor() {
  for (let i = 0; i < result.length; i++)
    result[
      i
    ].style = `fill: rgb(${currentColor[0]}, ${currentColor[1]}, ${currentColor[2]})`;
}
setLogoColor();
...
function isMouseOnSwitch(
  ...
  if (
    layerX > switchPosX &&
    layerX < switchPosX + switchSizeX &&
    layerY > switchPosY &&
    layerY < switchPosY + switchSizeY
  )
    return true;
  else return false;
}
...
function restrictSwitchToMoveOutRange(prop) {
  if (prop === "side")
    return (sideSwitchPos.y < 2 && sideDragEnd.y - sideDragStart.y < 0) ||
      (sideSwitchPos.y > 248 && sideDragEnd.y - sideDragStart.y > 0)
      ? 0
      : sideDragEnd.y - sideDragStart.y;
  if (prop === "y")
    return (mainSwitchPos.y < 2 && dragEnd.y - dragStart.y < 0) ||
      (mainSwitchPos.y > 248 && dragEnd.y - dragStart.y > 0)
      ? 0
      : dragEnd.y - dragStart.y;
  if (prop === "x")
    return (mainSwitchPos.x < 2 && dragEnd.x - dragStart.x < 0) ||
      (mainSwitchPos.x > 248 && dragEnd.x - dragStart.x > 0)
      ? 0
...
side_layer2.addEventListener("mousemove", e => {
  ...
  if (sideDrag) {
    ...
    drawColorLayer(
      `rgba(${sideColor[0]},${sideColor[1]},${sideColor[2]}, ${sideAlpha})`
    );
    drawBlackNWhiteLayer();
    setCurrentColor();
    draw();
